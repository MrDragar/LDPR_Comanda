...
@router.message(F.text == "Подтвердить авторизацию")
async def approve_auth_empty(message: types.Message):
    await message.answer("Укажите код подтверждения после команды.")


@router.message(RegistrationStates.auth_code)
...
@router.message(F.text == "Отклонить авторизацию")
async def decline_auth_empty(message: types.Message):
    await message.answer("Укажите код подтверждения после команды.")


@router.message(F.text.startswith("Подтвердить авторизацию"))
...
@router.message(F.text.startswith("Отклонить авторизацию"))
async def decline_auth_message(
        message: types.Message,
        notification_service: INotificationService
):
    token = (message.text or "").strip().split()[-1]
    answer = await decline_auth(token, message.from_user.id, Sources.TG, notification_service)
    await message.answer(answer)
