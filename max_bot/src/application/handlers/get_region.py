...
async def search_region(event: MessageCreated, context: MemoryContext, user_service: IUserService):
    ...
    if not regions:
        return await event.message.answer("Регион не найден. Попробуйте ввести название иначе.")

    await context.set_state(RegistrationStates.REGION_BY_BUTTON)
    await event.message.answer("Выберите ваш регион:",
                               attachments=[get_region_keyboard(regions).as_markup()])


@router.message_callback(F.callback.payload == "retry_reg")
async def retry_region_callback(event: MessageCallback, context: MemoryContext):
    if await context.get_state() not in [RegistrationStates.REGION_BY_TEXT,
                                         RegistrationStates.REGION_BY_BUTTON]: return
...
async def select_region_callback(event: MessageCallback, context: MemoryContext,
                                 user_service: IUserService):
    if await context.get_state() != RegistrationStates.REGION_BY_BUTTON: return
    ...
    await context.set_state(RegistrationStates.CITY)
    await event.message.answer("Укажите ваш город или населённый пункт:")
    await event.callback.answer()
