...
class AuthCommandRule(ABCRule[Message]):
    def __init__(self, prefix: str):
        self.prefix = prefix

    async def check(self, event: Message) -> bool:
        return (event.text or "").strip().startswith(self.prefix)
...
@router.message(text=["Подтвердить авторизацию"])
async def approve_auth_empty(message: Message):
    await message.answer("Укажите код подтверждения после команды.")


@router.message(state=RegistrationStates.AUTH_CODE)
...
@router.message(text=["Отклонить авторизацию"])
async def decline_auth_empty(message: Message):
    await message.answer("Укажите код подтверждения после команды.")


@router.message(AuthCommandRule("Подтвердить авторизацию"))
async def approve_auth_message(
    ...
    token = (message.text or "").strip().split()[-1]
...
@router.message(AuthCommandRule("Отклонить авторизацию"))
async def decline_auth_message(
        message: Message,
        notification_service: INotificationService
):
    token = (message.text or "").strip().split()[-1]
    answer = await decline_auth(token, message.from_id, Sources.VK, notification_service)
    await message.answer(answer)
