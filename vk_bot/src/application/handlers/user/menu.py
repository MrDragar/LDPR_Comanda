...
@router.message(text=["Поручения штаба"])
async def select_task_type(message: Message, user_service: IUserService,
    ...
    await message.answer("Выберите тип поручения:", keyboard=kb.get_json())
...
async def set_member_callback(event: GroupTypes.MessageEvent, user_service: IUserService,
    ...
    await event.ctx_api.messages.send(
        peer_id=event.object.peer_id,
        message="Выберите тип поручения:",
...
@router.message(text=["Действующие поручения"])
async def my_tasks(message: Message, offline_task_service, user_service,
    ...
    if not tasks:
        return await message.answer("У вас нет действующих поручений.")
    ...
    await message.answer("Ваши поручения:", keyboard=kb.get_json())
...
async def view_my_task(event: GroupTypes.MessageEvent, offline_task_service, user_service,
    ...
    if accepted and accepted.status == TaskStatus.IN_PROGRESS:
        kb.add(Callback("Отменить поручение", {"cmd": "cancel_my_task", "tid": tid}))
        kb.row()
...
async def cancel_my_task(event: GroupTypes.MessageEvent, offline_task_service, notification_service,
    ...
    try:
        ...
        await event.ctx_api.messages.send(
            peer_id=event.object.peer_id,
            message=f"Поручение #{tid} успешно отменено.",
            random_id=0,
        )
    except Exception as e:
        await event.ctx_api.messages.send(
...
async def back_to_my_tasks(event: GroupTypes.MessageEvent, offline_task_service, user_service,
    ...
    if not tasks:
        return await event.ctx_api.messages.send(
            peer_id=event.object.peer_id,
            message="У вас нет активных поручений.",
            random_id=0,
        )
